...
function generate_inactive_window_string() {
	...
	local separator_start="#[bg=${PALLETE['green2']},fg=${PALLETE['bg_highlight']}]${left_separator:?}#[none]"
	local separator_internal="#[bg=${PALLETE['dark3']},fg=${PALLETE['green2']}]${left_separator:?}#[none]"
	local separator_end="#[bg=${PALLETE[bg_highlight]},fg=${PALLETE['dark3']}]${left_separator:?}#[none]"

	echo "${separator_start}#[fg=${PALLETE['blue7']}]#I${separator_internal}#[fg=${PALLETE[white]}] #{?window_zoomed_flag, , }#W ${separator_end}"
}
...
function generate_active_window_string() {
	...
	separator_internal="#[bg=${PALLETE['blue6']},fg=${PALLETE['orange']}]${left_separator:?}#[none]"
	separator_end="#[bg=${PALLETE[bg_highlight]},fg=${PALLETE['blue6']}]${left_separator:?}#[none]"

	echo "${separator_start}#[fg=${PALLETE[terminal_black]}]#I${separator_internal}#[fg=${PALLETE[terminal_black]}] #{?window_zoomed_flag, , }#W ${separator_end}#[none]"
}
